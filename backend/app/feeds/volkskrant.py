"""
de Volkskrant RSS feed reader implementation.

Fetches and normalizes RSS feeds from de Volkskrant (quality broadsheet newspaper).
"""
...
class VolkskrantRssReader(FeedReader):
    """RSS reader for de Volkskrant news feeds."""

    def __init__(self, feed_url: str):
        """Initialize Volkskrant RSS reader."""
    ...
    @property
    def id(self) -> str:
        """Return unique identifier for Volkskrant feed reader."""
        return "volkskrant_rss"

    @property
    def source_metadata(self) -> Dict[str, Any]:
        """Return metadata about de Volkskrant as a news source."""
        return {
            "name": "de Volkskrant",
            "full_name": "de Volkskrant",
            "spectrum": "center-left",  # Volkskrant is considered progressive/center-left
            "country": "NL",
            "language": "nl",
            "media_type": "broadsheet"
        }
    ...
    async def fetch(self) -> List[FeedItem]:
        """
        Fetch and parse de Volkskrant RSS feed entries.
        ...
        """
        try:
            self.logger.info("Fetching Volkskrant RSS feed", feed_url=self.feed_url)
            ...
            self.logger.info("Successfully fetched Volkskrant feed",
            ...
            self.logger.error("Network error fetching Volkskrant RSS feed",
                            error=str(e), feed_url=self.feed_url)
            raise FeedReaderError(f"Network error fetching Volkskrant RSS: {e}")

        except httpx.HTTPStatusError as e:
            self.logger.error("HTTP error fetching Volkskrant RSS feed",
                            status_code=e.response.status_code,
                            error=str(e), feed_url=self.feed_url)
            raise FeedReaderError(f"HTTP error fetching Volkskrant RSS: {e}")

        except Exception as e:
            self.logger.error("Unexpected error fetching Volkskrant RSS feed",
                            error=str(e), feed_url=self.feed_url)
            raise FeedReaderError(f"Unexpected error fetching Volkskrant RSS: {e}")
    ...
    def _parse_entry(self, entry: Any, feed: Any) -> FeedItem:
        ...
        # Extract summary/description (note: Volkskrant RSS has empty descriptions)
        summary = None
        ...
        source_metadata = {
            **self.source_metadata,
            "feed_title": getattr(feed.feed, "title", "de Volkskrant"),
    ...
    def _extract_image_url(self, entry: Any) -> str | None:
        ...
        # Try media:content first (Volkskrant uses this format)
