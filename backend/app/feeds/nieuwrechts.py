"""
NieuwRechts RSS feed reader implementation.

Fetches and normalizes RSS feeds from NieuwRechts (Dutch right-wing news site).
"""
...
class NieuwRechtsRssReader(FeedReader):
    """RSS reader for NieuwRechts feeds."""

    @property
    def id(self) -> str:
        """Return unique identifier for NieuwRechts feed reader."""
        return "nieuwrechts_rss"

    @property
    def source_metadata(self) -> Dict[str, Any]:
        """Return metadata about NieuwRechts as a news source."""
        return {
            "name": "NieuwRechts",
            "full_name": "NieuwRechts",
            "spectrum": 9,  # Right-wing perspective (0=far-left, 10=far-right)
            "country": "NL",
            "language": "nl",
    ...
    async def fetch(self) -> List[FeedItem]:
        """
        Fetch and parse NieuwRechts RSS feed entries.
        ...
        """
        try:
            self.logger.info("Fetching NieuwRechts RSS feed", feed_url=self.feed_url)
            ...
            self.logger.info("Successfully fetched NieuwRechts feed",
            ...
            self.logger.error("Network error fetching NieuwRechts RSS feed",
                            error=str(e), feed_url=self.feed_url)
            raise FeedReaderError(f"Network error fetching NieuwRechts RSS: {e}")

        except httpx.HTTPStatusError as e:
            self.logger.error("HTTP error fetching NieuwRechts RSS feed",
                            status_code=e.response.status_code,
                            error=str(e), feed_url=self.feed_url)
            raise FeedReaderError(f"HTTP error fetching NieuwRechts RSS: {e}")

        except Exception as e:
            self.logger.error("Unexpected error fetching NieuwRechts RSS feed",
                            error=str(e), feed_url=self.feed_url)
            raise FeedReaderError(f"Unexpected error fetching NieuwRechts RSS: {e}")
    ...
    def _parse_entry(self, entry: Any, feed: Any) -> FeedItem:
        ...
        source_metadata = {
            **self.source_metadata,
            "feed_title": getattr(feed.feed, "title", "NieuwRechts"),
