"""
Het Parool RSS feed reader implementation.

Fetches and normalizes RSS feeds from Het Parool (Amsterdam's quality daily newspaper).
"""
...
class ParoolRssReader(FeedReader):
    ...
    @property
    def id(self) -> str:
        """Return unique identifier for Parool feed reader."""
        return "parool_rss"

    @property
    def source_metadata(self) -> Dict[str, Any]:
        """Return metadata about Het Parool as a news source."""
        return {
            "name": "Het Parool",
    ...
    async def fetch(self) -> List[FeedItem]:
        """
        Fetch and parse Het Parool RSS feed entries.
        ...
        try:
            ...
            self.logger.info("Successfully fetched Parool feed",
            ...
            self.logger.error("Network error fetching Parool RSS feed",
                            error=str(e), feed_url=self.feed_url)
            raise FeedReaderError(f"Network error fetching Parool RSS: {e}")

        except httpx.HTTPStatusError as e:
            self.logger.error("HTTP error fetching Parool RSS feed",
                            status_code=e.response.status_code,
                            error=str(e), feed_url=self.feed_url)
            raise FeedReaderError(f"HTTP error fetching Parool RSS: {e}")

        except Exception as e:
            self.logger.error("Unexpected error fetching Parool RSS feed",
                            error=str(e), feed_url=self.feed_url)
            raise FeedReaderError(f"Unexpected error fetching Parool RSS: {e}")
    ...
    def _parse_entry(self, entry: Any, feed: Any) -> FeedItem:
        ...
        # Extract summary/description (note: Parool RSS has empty descriptions like Volkskrant)
        summary = None
        ...
        source_metadata = {
            **self.source_metadata,
            "feed_title": getattr(feed.feed, "title", "Het Parool"),
    ...
    def _extract_image_url(self, entry: Any) -> str | None:
        ...
        # Try media:content first (Parool uses this format like Volkskrant)
